...
class ALOHAWriterForSpenso(ALOHAWriterForCPP):
    ...
    def _strip_evaluator_includes(self, evaluator_source):
        """Remove includes when embedding the evaluator into a larger C++ unit."""

        lines = [
            line for line in evaluator_source.splitlines()
    ...
    def write(self, mode=None):
        output = self._define_spenso_output(mode=mode)
...
class ALOHAWriterForSpensoCPP(ALOHAWriterForSpenso):
    """Expose the spenso evaluator through the normal C++ writer contract."""

    def write(self, mode=None):
        include_evaluator_includes = mode != 'no_include'
        cc_text = self._define_spenso_output(
            mode=mode,
            include_evaluator_includes=include_evaluator_includes
        )
        h_text = self.get_h_text()
...
class WriterFactory(object):
    ...
    def __new__(cls, data, language, outputdir, tags, options=None):
        ...
        if language == 'fortran':
            ...
            if options and options.get('cpp_backend') == 'spenso':
                return ALOHAWriterForSpensoCPP(data, outputdir, options=options)
